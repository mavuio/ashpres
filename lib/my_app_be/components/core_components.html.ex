defmodule MyAppBe.CoreComponents do
  @moduledoc """
  ...
  def mybutton(assigns) do
    default_classes = "btn-outline inline-flex justify-center items-center space-x-2"

    assigns
    |> assign(:class, [default_classes, assigns[:class] || ""])
    |> assign_new(:tabindex, fn -> 0 end)
    |> case do
      %{type: type} = assigns when type in ["button", "submit"] ->
        rest = assigns_to_attributes(assigns, [:disabled])

        rest =
          if assigns[:disabled] == false do
            rest ++ [disabled: false]
          else
            rest
          end
