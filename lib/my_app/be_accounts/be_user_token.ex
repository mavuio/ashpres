defmodule MyApp.BeAccounts.BeUserToken do
  use Ecto.Schema
  import Ecto.Query
  alias MyApp.BeAccounts.BeUserToken
  ...
  @init_password_validity_in_days 30
  ...
  Therefore, storing them allows individual be_user
  ...
  def build_session_token(be_user) do
    ...
    {token, %BeUserToken{token: token, context: "session", be_user_id: be_user.id}}
  end

  @doc """
  Checks if the token is valid and returns its underlying lookup query.

  The query returns the be_user found by the token, if any.
  ...
  @doc """
  Builds a token and its hash to be delivered to the be_user's email.
  ...
  defp days_for_context("init_password"), do: @init_password_validity_in_days
  ...
  This is used to validate requests to change the be_user
  ...
  def token_and_context_query(token, context) do
    from BeUserToken, where: [token: ^token, context: ^context]
  end
  ...
  def be_user_and_contexts_query(be_user, :all) do
    from t in BeUserToken, where: t.be_user_id == ^be_user.id
  end

  def be_user_and_contexts_query(be_user, [_ | _] = contexts) do
    from t in BeUserToken, where: t.be_user_id == ^be_user.id and t.context in ^contexts
  end
end
